...
resource "aws_iam_role_policy" "nomad_ec2" {
  name   = "${var.friendly_name_prefix}-nomad-controller-instance-role-policy-${data.aws_region.current.region}"
  role   = aws_iam_role.nomad_ec2.id
  policy = data.aws_iam_policy_document.combined.json
}

resource "aws_iam_role" "nomad_ec2" {
  name = "${var.friendly_name_prefix}-nomad-instance-role-${data.aws_region.current.region}"
  path = "/"
...
resource "aws_iam_instance_profile" "nomad_ec2" {
  name = "${var.friendly_name_prefix}-nomad-${data.aws_region.current.region}"
  path = "/"
